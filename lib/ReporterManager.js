define([
	...
	'dojo/has!host-node?dojo/node!mkdirp',
	'dojo/has!host-node?dojo/node!istanbul/lib/report/common/defaults',
	'dojo/lang',
	'dojo/aspect',
	'dojo/Promise'
], function (has, fs, path, mkdirp, istanbulDefaults, lang, aspect, Promise) {
	function noop() {}
		...
		add: function (Reporter, config) {
			...
			if (typeof Reporter === 'object') {
				...
				if (has('host-node')) {
					/* jshint node:true */
					if (config.filename) {
						if (path.dirname(config.filename) !== '.') {
							mkdirp.sync(path.dirname(config.filename));
						}
